...
void addfd(int epollfd, int fd, bool one_shot, int TRIGMode)
{
    ...
    if (one_shot)
        event.events |= EPOLLONESHOT; //使一个socket连接任何时刻都只被一个线程所处理
...
//将事件重置为EPOLLONESHOT
//如果注册了EPOLLONESHOT的socket一旦被某个线程处理完毕，
//则应该立即重置这个socket上的EPOLLONESHOT事件
